...
export default function EmailLoginForm() {
    ...
    const r = searchParams.get("r") || searchParams.get("redirect");
    const redirectUrl = r ? (r.startsWith("/") ? r : `/${r}`) : null;
    ...
    // If already logged in, redirect immediately (don't show login form)
    useEffect(() => {
        if ((token || isAuthenticated) && user) {
            const target = redirectUrl && redirectUrl.startsWith("/") ? redirectUrl : user.role === 2 ? "/joinUsEvent" : "/events";
            window.location.replace(target);
            return;
        }
    }, [token, user, isAuthenticated, redirectUrl]);
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                if (response?.status === 1) {
                    toast.success(t("OTP.otpSentToPhone") || response.message || "OTP sent successfully to your phone number");
                    setStep(2); // Move to OTP verification step
                    setOtpSentTime(Date.now()); // Record when OTP was sent
                    setOtpExpired(false);
                    startTimer();
                } else {
                    toast.error(t("OTP.failedToSendOTP") || response.message || "Failed to send OTP");
                }
            } catch (error) {
                setLoading(false);
        ...
        onSubmit: async (values) => {
            ...
            try {
                ...
                if (response?.status === 1) {
                    ...
                    // Store token (path: '/' so middleware receives it on all routes)
                    Cookies.set(TOKEN_NAME, token, { expires: 30, path: '/' });
                    ...
                    toast.success(t("auth.loginSuccess") || response.message || "Login successful!");

                    // Hard redirect: form closes immediately, user lands on role-based page
                    const targetUrl = redirectUrl && redirectUrl.startsWith("/") ? redirectUrl : user.role === 2 ? "/joinUsEvent" : "/events";
    ...
    const resendCode = () => {
        ...
        SendPhoneOTPApi({
            ...
            if (data?.status === 1) {
                ...
                toast.success(t("OTP.otpResentSuccess") || data?.message || "OTP resent successfully!");
                startTimer();
            } else {
                toast.error(t("OTP.failedToResendOTP") || data?.message || "Failed to resend OTP");
            }
        }).catch((error) => {
            setLoading(false);
    ...
    // After all hooks: if already authenticated, show loader until redirect (avoids hooks order error)
    if ((token || isAuthenticated) && user) {
        return (
    ...
    return (
        ...
        >
            <div className="relative bg-gradient-to-br from-white via-white to-purple-50/30 rounded-3xl shadow-2xl p-8 border border-gray-100/50 backdrop-blur-sm overflow-hidden">
                {/* Animated background elements - pointer-events-none so they don't block button clicks */}
                <div className="absolute top-0 right-0 w-72 h-72 bg-purple-200/20 rounded-full blur-3xl -translate-y-1/2 translate-x-1/2 pointer-events-none"></div>
                <div className="absolute bottom-0 left-0 w-56 h-56 bg-orange-200/20 rounded-full blur-3xl translate-y-1/2 -translate-x-1/2 pointer-events-none"></div>
                ...
                <div className="relative z-10">
                    ...
                    >
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <a
                                href="/signup/guest"
                                className="block group py-2.5 px-3 border-2 border-brand-orange text-brand-orange hover:bg-gradient-to-r hover:from-brand-orange/10 hover:to-orange-50 font-semibold rounded-xl transition-all duration-300 shadow-sm hover:shadow-md text-center no-underline"
                            >
                            ...
                            <a
                                ...
                                className="block group py-2.5 px-3 bg-gradient-to-r from-[#a797cc] to-[#9d8bc0] hover:from-[#9d8bc0] hover:to-[#a797cc] text-white font-semibold rounded-xl transition-all duration-300 shadow-md hover:shadow-lg text-center no-underline"
                            >
                                <span className="flex items-center justify-center gap-2">
