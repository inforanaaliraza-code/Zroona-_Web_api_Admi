 ...
 */
  ...
  hideCountryCodeInUI = true,
  ...props 
}, ref) => {
  const { i18n } = useTranslation();
  const phoneInputRef = useRef(null);
  const didSetCountryCode = useRef(false);
  const isRTL = i18n.language === 'ar';
  const direction = isRTL ? 'rtl' : 'ltr';

  // Simple mode: set country_code to +966 once on mount
  useEffect(() => {
    if (hideCountryCodeInUI && formik && !didSetCountryCode.current) {
      didSetCountryCode.current = true;
      formik.setFieldValue(countryCodeField, '+966');
    }
  }, [hideCountryCodeInUI, formik, countryCodeField]);
  ...
  // Simple input mode: no flag/country code in UI, placeholder (5xxxxxxxx), +966 sent to backend
  const handleSimpleChange = (e) => {
    const raw = e.target.value.replace(/\D/g, '').slice(0, 9);
    if (formik) {
      formik.setFieldValue(mobileNumberField, raw);
      formik.setFieldValue(countryCodeField, '+966');
    }
    if (onChange) onChange(raw, { dialCode: '966' }, e, raw);
  };

  if (hideCountryCodeInUI && formik) {
    const simpleValue = formik.values[mobileNumberField] || '';
    return (
      <div className={`relative w-full max-w-full ${className}`} dir={direction} ref={ref}>
        <input
          type="tel"
          inputMode="numeric"
          pattern="[0-9]*"
          ...
          className="w-full h-[56px] rounded-xl border border-[#f2dfba] bg-[#fdfdfd] px-4 text-gray-900 text-base placeholder:text-gray-400 placeholder:opacity-60 focus:border-[#a797cc] focus:outline-none focus:ring-[3px] focus:ring-[#a797cc]/20 disabled:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-60"
          style={{
        ...
        />
        {error && (
          <p className="mt-1 text-sm text-red-500" dir={direction}>{error}</p>
        )}
