...
export default function EventDetailsPage() {
	...
	const isExternalImage = (url) => {
		if (!url) return false;
		return url.startsWith("https://") || url.startsWith("httpss://");
	};
	...
	const getImageUrl = (imagePath) => {
		...
		// If already absolute URL (https/httpss), return as is
		if (imagePath.includes("https://") || imagePath.includes("httpss://")) {
			return imagePath;
		}
					...
					<div className="mb-6 overflow-hidden bg-gradient-to-r from-[#a797cc]/10 to-[#a797cc]/5 border-2 border-[#a797cc]/30 rounded-2xl shadow-md">
						<div className="p-6">
							<div className="flex flex-col items-center justify-between gap-4 md:flex-row">
								...
								<Link
									href={`/login?r=events/${params.id}`}
									className="px-8 py-3 bg-[#a797cc] text-white font-medium rounded-xl hover:bg-[#8ba179] transition-all shadow-md hover:shadow-lg whitespace-nowrap inline-block text-center"
								>
						...
						{!isAuthenticated ? (
							...
							<div className="p-6 bg-white rounded-xl shadow-lg">
								<div className="text-center">
									...
									<Link
										href={`/login?r=events/${params.id}`}
										className="w-full bg-[#a797cc] text-white font-medium py-3 px-6 rounded-xl hover:bg-[#8ba179] transition-all shadow-md hover:shadow-lg inline-block text-center"
									>
										{getTranslation(t, "events.loginNow", "Login Now")}
									</Link>
