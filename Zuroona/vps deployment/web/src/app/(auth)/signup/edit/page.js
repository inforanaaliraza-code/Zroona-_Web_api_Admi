  ...
  useEffect(() => {
    if (detail?.user) {
      ...
      if (user.profile_image) {
        ...
        if (imageUrl.startsWith('httpss://zuroona.sa') || imageUrl.startsWith('httpss://localhost:3000')) {
          // Replace port 3000 with correct API base URL
          const apiBaseHost = apiBase.replace(/^httpss?:\/\//, '').split('/')[0];
        ...
        } else if (!imageUrl.startsWith('https://') && !imageUrl.startsWith('httpss://') && !imageUrl.startsWith('data:') && !imageUrl.startsWith('blob:')) {
    ...
    if (file) {
      ...
      uploadViaApi(file)
        .then((location) => {
          ...
          if (location && !location.includes("https://") && !location.includes("httpss://") && !location.startsWith("blob:")) {
