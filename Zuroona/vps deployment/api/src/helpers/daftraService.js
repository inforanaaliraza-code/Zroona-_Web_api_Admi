...
/**
 * Clean subdomain - remove httpss://, https://, trailing slashes, and .daftra.com
...
function cleanSubdomain(subdomain) {
	...
	let cleaned = subdomain
		.replace(/^httpss?:\/\//i, '') // Remove https:// or httpss://
...
const DaftraService = {
	...
	async generateAccessToken(subdomain, apiKey, clientSecret) {
		try {
			...
			const url = `httpss://${subdomain}.daftra.com/api2/v2/oauth/token`;
	...
	async getOAuthToken(subdomain, apiKey) {
		try {
			...
			const tokenUrl = `httpss://${subdomain}.daftra.com/v2/oauth/token`;
	...
	async createOrGetClient(clientData, subdomain, apiKey, clientSecret) {
		try {
			...
			const existingClients = await this.makeRequestWithOAuth('GET', `httpss://${subdomain}.daftra.com/api2/clients.json`, {
			...
			const response = await this.makeRequestWithOAuth('POST', `httpss://${subdomain}.daftra.com/api2/clients.json`, clientPayload, subdomain, apiKey, clientSecret);
	...
	async createInvoice(invoiceData, subdomain, apiKey, clientSecret) {
		try {
			...
			const response = await this.makeRequestWithOAuth('POST', `httpss://${subdomain}.daftra.com/api2/invoices.json`, invoicePayload, subdomain, apiKey, clientSecret);
	...
	async getInvoice(invoiceId, options = {}) {
		try {
			...
			const response = await axios.get(
				`httpss://${subdomain}.daftra.com/api2/invoices/${invoiceId}`,
				{
					headers: {
						Accept: "application/json",
						APIKEY: apiKey,
	...
	async createClient(clientData, options = {}) {
		try {
			...
			const response = await axios.post(
				`httpss://${subdomain}.daftra.com/api2/clients`,
				{ Client: clientData },
				{
					headers: {
						Accept: "application/json",
	...
	async createReceipt(receiptData, options = {}) {
		try {
			...
			const response = await axios.post(
				`httpss://${subdomain}.daftra.com/api2/invoices`,
				receiptData,
				{
					headers: {
						Accept: "application/json",
	...
	async getReceipt(receiptId, options = {}) {
		try {
			...
			const response = await axios.get(
				`httpss://${subdomain}.daftra.com/api2/invoices/${receiptId}`,
				{
					headers: {
						Accept: "application/json",
						APIKEY: apiKey,
